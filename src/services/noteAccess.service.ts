 ...
 * note it points to, same as an anonymous visitor already can. */
...
export async function getNoteAccess(userId: string, noteId: string): Promise<NoteAccessResult | null> {
  ...
  const publicEdit = !!publicShare?.enabled && publicShare.access === "edit";
